...
export function findAirportsName(letter: string, maxResults: number = 8): Airport[] {
    const lowerCaseLetter = letter.toLowerCase();

    return airportsList
        .filter((airport) =>
            airport.code.toLowerCase().includes(lowerCaseLetter) ||
            airport.name.toLowerCase().includes(lowerCaseLetter) ||
            airport.city.toLowerCase().includes(lowerCaseLetter)
        )
        .slice(0, maxResults);
}
