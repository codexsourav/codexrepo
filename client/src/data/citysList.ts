...
// Filter the array based on the condition and limit the results
export function findCities(letter: string, maxResults: number = 8): City[] {
    const lowerCaseLetter = letter.toLowerCase();

    const filteredCities = citysList.filter((city) =>
        city.name.toLowerCase().includes(lowerCaseLetter) ||
        city.state.toLowerCase().includes(lowerCaseLetter)
    ).slice(0, maxResults);
